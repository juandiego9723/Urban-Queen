...
echo [1/3] Creando lanzador silencioso de servidor (Iniciar_TikDance.vbs)...
(
...
echo [2/3] Creando script de detencion limpia (Detener_TikDance.vbs)...
(
echo Set WshShell = CreateObject("WScript.Shell"^)
...
echo [3/3] Generando acceso directo "TikDance" con icono oficial en el Escritorio...
(
echo Set WshShell = CreateObject("WScript.Shell"^)
echo desktopPath = WshShell.SpecialFolders("Desktop"^)
echo Set shortcut = WshShell.CreateShortcut(desktopPath ^& "\TikDance.lnk"^)
echo shortcut.TargetPath = "%CURRENT_DIR%\Iniciar_TikDance.vbs"
echo shortcut.WorkingDirectory = "%CURRENT_DIR%"
echo shortcut.IconLocation = "%CURRENT_DIR%\public\app-icon.ico"
